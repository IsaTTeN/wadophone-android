...
import org.wadophone.contacts.ContactLoader.Companion.LINPHONE_ADDRESS_BOOK_FRIEND_LIST
...
class CorePreferences
    ...
    var automaticallyShowDialpad: Boolean
        ...
        // DTMF sequence to send to open the door lock/intercom while in a call, e.g. "1,2,3"
    @get:AnyThread @set:WorkerThread
    var doorOpenerDtmfSequence: String
        get() = config.getString("ui", "door_opener_dtmf_sequence", "").orEmpty()
        set(value) {
            config.setString("ui", "door_opener_dtmf_sequence", value)
        }

    // Whether the door opener button should be displayed in the in-call action bar
    @get:AnyThread @set:WorkerThread
    var doorOpenerButtonEnabled: Boolean
        get() = config.getBool("ui", "door_opener_button_enabled", true)
        set(value) {
            config.setBool("ui", "door_opener_button_enabled", value)
        }

    // Whether the in-call action buttons bar (hang up, mute, speaker, door opener, ...) should
    // stay visible automatically as soon as the call starts, instead of only appearing after the
    // user taps the screen (which is the default behavior, e.g. when full-screen video kicks in)
    @get:AnyThread @set:WorkerThread
    var autoShowCallActionsBar: Boolean
        get() = config.getBool("ui", "auto_show_call_actions_bar", false)
        set(value) {
            config.setBool("ui", "auto_show_call_actions_bar", value)
        }
    ...
    private fun copy(from: String, to: String, overrideIfExists: Boolean = false) {
        ...
        if (outFile.exists()) {
            if (!overrideIfExists) {
                android.util.Log.i(
                    context.getString(org.wadophone.R.string.app_name),
