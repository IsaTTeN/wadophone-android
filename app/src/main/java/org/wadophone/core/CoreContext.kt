...
import org.wadophone.BuildConfig
...
import org.wadophone.contacts.ContactsManager
import org.linphone.core.tools.Log
import org.wadophone.notifications.NotificationsManager
import org.wadophone.telecom.TelecomManager
import org.wadophone.ui.call.CallActivity
import org.wadophone.utils.ActivityMonitor
...
class CoreContext
    ...
    private val coreListener = object : CoreListenerStub() {
        ...
        override fun onConfiguringStatus(
            ...
            if (status == ConfiguringState.Successful) {
                ...
                showGreenToastEvent.postValue(
                    Event(
                        Pair(
                            org.wadophone.R.string.remote_provisioning_config_applied_toast,
                            org.wadophone.R.drawable.smiley
                        )
                    )
                )
                ...
                showRedToastEvent.postValue(
                    Event(
                        Pair(
                            org.wadophone.R.string.remote_provisioning_config_failed_toast,
                            org.wadophone.R.drawable.warning_circle
                        )
                    )
                )
            }
            ...
            when (currentState) {
                ...
                Call.State.Error -> {
                    ...
                    showFormattedRedToastEvent.postValue(
                        Event(Pair(text, org.wadophone.R.drawable.warning_circle))
                    )
                }
                else -> {
                }
        ...
        override fun onTransferStateChanged(core: Core, transfered: Call, state: Call.State) {
            ...
            if (state == Call.State.Connected) {
                val icon = org.wadophone.R.drawable.phone_transfer
                showGreenToastEvent.postValue(
                    Event(Pair(org.wadophone.R.string.call_transfer_successful_toast, icon))
                )
            }
        }
    ...
    override fun run() {
        ...
        val gitVersion = AppUtils.getString(org.wadophone.R.string.linphone_app_version)
        val gitBranch = AppUtils.getString(org.wadophone.R.string.linphone_app_branch)
    ...
    fun computeUserAgent() {
        ...
        val appName = context.getString(org.wadophone.R.string.app_name)
