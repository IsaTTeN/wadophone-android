 ...
 */
package org.wadophone.ui.call.viewmodel
...
import org.wadophone.ui.call.conference.viewmodel.ConferenceViewModel
import org.wadophone.ui.call.model.AudioDeviceModel
import org.wadophone.ui.call.model.CallMediaEncryptionModel
import org.wadophone.ui.call.model.CallStatsModel
import org.wadophone.ui.main.contacts.model.ContactAvatarModel
import org.wadophone.ui.main.history.model.NumpadModel
import org.wadophone.utils.AppUtils
import org.wadophone.utils.AudioUtils
...
class CurrentCallViewModel
    ...
    companion object {
        ...
        private const val DOOR_OPENER_DTMF_DELAY_MS = 500L
    }
    ...
    val showDoorOpenerButton = MutableLiveData<Boolean>()
    ...
    init {
        ...
        coreContext.postOnCoreThread { core ->
            ...
            showDoorOpenerButton.postValue(
                corePreferences.doorOpenerDtmfSequence.isNotEmpty() &&
                    corePreferences.doorOpenerButtonEnabled
            )
    ...
    @UiThread
    fun openDoor() {
        val sequence = corePreferences.doorOpenerDtmfSequence
            .split(",")
            .map { it.trim() }
            .filter { it.isNotEmpty() }
        if (sequence.isEmpty()) {
            Log.w("$TAG No door opener DTMF sequence configured, doing nothing")
            return
        }

        viewModelScope.launch {
            for ((index, digits) in sequence.withIndex()) {
                coreContext.postOnCoreThread {
                    if (::currentCall.isInitialized) {
                        for (digit in digits) {
                            Log.i("$TAG Sending door opener DTMF [$digit]")
                            currentCall.sendDtmf(digit)
                        }
                    }
                }
                if (index < sequence.size - 1) {
                    delay(DOOR_OPENER_DTMF_DELAY_MS)
                }
            }
        }
    }
    ...
    private fun updateVideoDirection(direction: MediaDirection, skipIfNotStreamsRunning: Boolean = false) {
        ...
        if (currentCall.conference == null) { // Let conference view model handle full screen while in conference
            if (isReceiving && !wasReceiving) { // Do not change full screen mode base on our video being sent when it wasn't
                if (fullScreenMode.value != true && !corePreferences.autoShowCallActionsBar) {
                    Log.i(
