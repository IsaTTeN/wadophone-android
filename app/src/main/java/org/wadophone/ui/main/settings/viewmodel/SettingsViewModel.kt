 ...
 */
package org.wadophone.ui.main.settings.viewmodel
...
import org.wadophone.contacts.ContactLoader.Companion.NATIVE_ADDRESS_BOOK_FRIEND_LIST
...
import org.wadophone.core.VFS
import org.linphone.core.tools.Log
import org.wadophone.ui.GenericViewModel
import org.wadophone.ui.main.settings.model.CardDavLdapModel
import org.wadophone.ui.main.settings.model.CodecModel
...
class SettingsViewModel
    ...
    val doorOpenerCode = MutableLiveData<String>()

    val doorOpenerButtonEnabled = MutableLiveData<Boolean>()

    val autoShowCallActionsBar = MutableLiveData<Boolean>()
    ...
    init {
        ...
        coreContext.postOnCoreThread { core ->
            ...
            doorOpenerCode.postValue(corePreferences.doorOpenerDtmfSequence)
            doorOpenerButtonEnabled.postValue(corePreferences.doorOpenerButtonEnabled)
            autoShowCallActionsBar.postValue(corePreferences.autoShowCallActionsBar)
    ...
    @UiThread
    fun toggleDoorOpenerButton() {
        val newValue = doorOpenerButtonEnabled.value == false
        coreContext.postOnCoreThread {
            corePreferences.doorOpenerButtonEnabled = newValue
            doorOpenerButtonEnabled.postValue(newValue)
        }
    }

    @UiThread
    fun toggleAutoShowCallActionsBar() {
        val newValue = autoShowCallActionsBar.value == false
        coreContext.postOnCoreThread {
            corePreferences.autoShowCallActionsBar = newValue
            autoShowCallActionsBar.postValue(newValue)
        }
    }

    @UiThread
    fun updateDoorOpenerCode() {
        coreContext.postOnCoreThread {
            val newCode = doorOpenerCode.value.orEmpty().trim()
            if (newCode != corePreferences.doorOpenerDtmfSequence) {
                corePreferences.doorOpenerDtmfSequence = newCode
                Log.i("$TAG Updated door opener DTMF sequence to [$newCode]")
            }
        }
    }
