...
import org.wadophone.databinding.AssistantPermissionsFragmentBinding
import org.wadophone.ui.GenericFragment
import org.wadophone.ui.assistant.AssistantActivity
import org.wadophone.ui.assistant.viewmodel.PermissionsViewModel
...
class PermissionsFragment : GenericFragment() {
    ...
    private fun leave() {
        ...
        if (requireActivity().intent.getBooleanExtra(AssistantActivity.PERMISSIONS_ONLY_EXTRA, false)) {
            Log.w(
                "$TAG Activity was only started to request permissions, leaving now that they've been granted"
            )
            try {
