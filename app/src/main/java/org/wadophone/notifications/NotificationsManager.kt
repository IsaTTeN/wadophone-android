 ...
 */
package org.wadophone.notifications
...
import org.wadophone.contacts.AvatarGenerator
import org.wadophone.contacts.ContactsManager.ContactsListener
import org.wadophone.contacts.getAvatarBitmap
import org.wadophone.contacts.getPerson
...
import org.wadophone.core.CoreInCallService
import org.wadophone.core.CoreKeepAliveThirdPartyAccountsService
...
import org.wadophone.ui.call.CallActivity
import org.wadophone.ui.main.MainActivity
import org.wadophone.ui.main.MainActivity.Companion.ARGUMENTS_CHAT
import org.wadophone.ui.main.MainActivity.Companion.ARGUMENTS_CONVERSATION_ID
import org.wadophone.utils.AppUtils
import org.wadophone.utils.FileUtils
import org.wadophone.utils.LinphoneUtils
import org.wadophone.utils.ShortcutUtils
...
class NotificationsManager
    ...
    companion object {
        ...
        const val INTENT_HANGUP_CALL_NOTIF_ACTION = "org.wadophone.HANGUP_CALL_ACTION"
        const val INTENT_ANSWER_CALL_NOTIF_ACTION = "org.wadophone.ANSWER_CALL_ACTION"
        const val INTENT_TOGGLE_SPEAKER_CALL_NOTIF_ACTION = "org.wadophone.TOGGLE_SPEAKER_CALL_ACTION"
        const val INTENT_REPLY_MESSAGE_NOTIF_ACTION = "org.wadophone.REPLY_ACTION"
        const val INTENT_MARK_MESSAGE_AS_READ_NOTIF_ACTION = "org.wadophone.MARK_AS_READ_ACTION"
        const val INTENT_FOREGROUND_SERVICE_NOTIF_DISMISSED_ACTION = "org.wadophone.INTENT_FOREGROUND_SERVICE_NOTIF_DISMISSED_ACTION"
        const val INTENT_ALLOW_CALL_REDIRECTION_ACTION = "org.wadophone.INTENT_ALLOW_CALL_REDIRECTION_ACTION"
        const val INTENT_DENY_CALL_REDIRECTION_ACTION = "org.wadophone.INTENT_DENY_CALL_REDIRECTION_ACTION"
        ...
        const val INTENT_ANSWER_CALL_EXTRA = "AnswerCall"
    ...
    private fun showDummyNotificationForCallService() {
        ...
        if (service != null) {
            ...
            val builder = NotificationCompat.Builder(context, channelId)
                .setSmallIcon(R.drawable.wadophone_notification)
                .setAutoCancel(false)
                .setOngoing(true)
    ...
    fun getCallAnswerPendingIntent(notifiable: Notifiable): PendingIntent {
        // Using a direct Activity PendingIntent (instead of a Broadcast one) so the OS treats
        // this exactly like a notification tap: it is guaranteed to be allowed to bring
        // CallActivity to the foreground even if the device is unlocked and in use, without
        // being subject to background activity launch restrictions that would otherwise delay
        // or silently block starting the Activity once the call is actually answered.
        val answerIntent = Intent(context, CallActivity::class.java)
        answerIntent.apply {
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_REORDER_TO_FRONT)
            putExtra("IncomingCall", true)
            putExtra(INTENT_ANSWER_CALL_EXTRA, true)
            putExtra(INTENT_NOTIF_ID, notifiable.notificationId)
            putExtra(INTENT_REMOTE_SIP_URI, notifiable.remoteAddress)
        }
        val options = Compatibility.getPendingIntentActivityOptions(true)
